...
export function Produk() {
  ...
  const pathProductId = useMemo(() => {
    ...
    const match = location.pathname.match(/^\/produk\/([^/]+)$/)
    return match?.[1]
  }, [productId, location.pathname])

  const isDetailView = Boolean(pathProductId)
  const selectedProductId = isDetailView ? pathProductId || null : null
    ...
    <PageLayout>
      ...
      {isDetailView ? (
        <section className="mx-auto w-full max-w-6xl px-6 pb-16 pt-12 md:pb-20 md:pt-16">
          ...
          <div className="mt-6 grid items-center gap-10 md:grid-cols-[1.05fr_0.95fr]">
